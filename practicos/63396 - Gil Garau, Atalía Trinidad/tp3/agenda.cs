...
public sealed class AgendaWindow : Runnable
{
    ...
    private void BuildLayout()
    {
        MenuBar menu = new()
        {
            Menus = [
                ...
                new MenuBarItem("_Ayuda", [
                    new MenuItem("_Atajos", null, () => MessageBox.Query(App!, "Atajos", "F2 nuevo | F3 editar | Del borrar | Ctrl+I importar | Ctrl+E exportar | Ctrl+Q salir", "OK")),
                    null!,
                    new MenuItem("_Acerca de", null, () => MessageBox.Query(App!, "Acerca de", "Agenda TUI simple", "OK"))
                ])
            ]
        };
        ...
        Add(menu, searchLabel, search, left, right);
    }
    ...
    private void ExportJson()
    {
        ...
        if (path is null) return;
        try { JsonAgendaIO.Write(path, store.GetAll()); }
    ...
    private void RefreshList()
    {
        ...
        filtered = contacts.Where(c => (!onlyFav || c.Favorito) && (q.Length == 0 || c.Nombre.Contains(q, StringComparison.OrdinalIgnoreCase) || c.Telefonos.Contains(q, StringComparison.OrdinalIgnoreCase) || (c.Email?.Contains(q, StringComparison.OrdinalIgnoreCase) ?? false))).ToList();
...
{
    readonly TextField nombre = new(), email = new(), telefono = new(), favorito = new();
    ...
    public EjemploDialog(string title, Contacto c)
    {
        ...
        Add(new Label { Text = "Completa nombre y telefono. Email opcional.", X = 2, Y = 0 });
        AddRow("Nombre:", 1, nombre, c.Nombre);
        AddRow("Email:", 2, email, c.Email ?? "");
        AddRow("Telefono:", 3, telefono, c.Telefonos);
        AddRow("Favorito (s/n):", 4, favorito, c.Favorito ? "s" : "n");
